...
public class BeanDataSourceResourceValidator extends GenericResourceValidator<BeanReportDataSource> {
    @Override
    protected void internalValidate(BeanReportDataSource resource, ValidationErrors errors) {
        if (empty(resource.getBeanMethod())){
            addMandatoryParameterNotFoundError(errors, "BeanMethod");
        }
        if (empty(resource.getBeanName())){
            addMandatoryParameterNotFoundError(errors, "BeanName");
        }
    }
}
