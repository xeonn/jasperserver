 ...
 * @version $Id: TxtFileConnection.java 44312 2014-04-09 14:30:12Z vsabadosh $
 */
...
public class TxtFileConnection extends AbstractFileConnection<TxtFileConnection> {
    ...
    public TxtFileConnection(TxtFileConnection source){
        super(source);
        if(source.getParser() instanceof TxtFileDelimiterParser){
            parser = new TxtFileDelimiterParser((TxtFileDelimiterParser) source.getParser());
        } else if(source.getParser() instanceof TxtFileRegularExpressionParser){
            parser = new TxtFileRegularExpressionParser((TxtFileRegularExpressionParser) source.getParser());
        }
    }
