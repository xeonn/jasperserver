...
/**
 * @version $Id: jive.js 2858 2014-11-18 23:20:15Z dgorbenko $
 */

define(["common/jquery/extension/timepickerExt", 'text!jr.jive.templates.tmpl', 'csslink!jr.jive.vm.css', 'text!jr.jive.i18n.tmpl'], function($, templates, css, jivei18nText) {
    ...
    var jive = {
        ...
        ui: {
            ...
            marker: {
                ...
                createElement: function() {
                    var $marker = $("<div/>", {id: "jive_marker"}).css({display: "none"}),
                        $markerIcon = $("<div/>", {id: "jive_marker_icon"}),
                        $markerContent = $("<div/>").css({
                            position: "absolute",
                            top: 0,
                            right: "6px",
                            bottom: 0,
                    ...
                    jive.$jiveComponents.append($marker.append($markerIcon).append($markerContent));
                },
                setElement: function(selector){
                    $(selector).length && this.createElement();
                    this.jo = $(selector);
                    this.jo.draggable({
                        axis: "x",
                        ...
                        stop:function(ev,ui) {
                            jive.interactive[jive.selected.ie.config.type].resize((ui.position.left - jive.ui.overlay.left) / (jive.reportInstance.zoom ? jive.reportInstance.zoom.level : (jive.ui.scaleFactor ? jive.ui.scaleFactor : 1)));
                ...
                show: function(dim){
                    ...
                    if (this.jo && !this.jo.parent().length) {
                        delete this.jo;
                    }
                    if (!this.jo) {
                    ...
                    // this will be used later on in the 'jive.interactive.column.js' file
            ...
            overlay: {
                ...
                left: null,
                createElement: function() {
                    var $dragLabel = $("<div/>", {id: "jive_drag_label"}).css({
                        background:"#eee",
                        border:"solid 1px #555",
                        padding:"8px",
                        display:"none"
                    }),
                    $overlay = $("<div/>", {id: "jive_overlay"}).css({display: "none"}).append("<div/>");

                    jive.$jiveComponents.append($overlay).append($dragLabel);
                },
                setElement: function(selector){
                    $(selector).length && this.createElement();
                    this.jo = $(selector);
            ...
            foobar: {
                ...
                menus: {},
                createElement: function() {
                    var $foobar = $("<div/>", {id: "jive_foobar"}).css({display: "none"});
                    jive.$jiveComponents.append($foobar);
                },
                setElement: function(selector){
                    $(selector).length && this.createElement();
                    this.jo = $(selector);
                ...
                setToolbarPosition: function() {
                    ...
                    var jiveMenuHeight = this.jo.outerHeight();
                    var offsetDifference = jive.ui.overlay.jo.offset().top - jive.getReportContainer().offset().top;
                    var additionalOffset = offsetDifference <= jiveMenuHeight  ? jiveMenuHeight : 0;

                    this.jo.css({position: 'absolute'});
                    this.jo.offset({top: top+additionalOffset, left: jive.selected.jo.offset().left});
                ...
                remove: function() {
                    if (this.jo) {
                        this.jo.off().remove();
                        this.dropMenu && this.dropMenu.jo.remove();
                        this.jo = null;
                        this.reset();
                    }
                }
            ...
            dialog: {
                ...
                setElement: function(selector){
                     ...
                     */
                    it.body = it.jo.find('div.dialogBody');
                    it.title = it.jo.find('span.dialogTitle');
                    it.tabs = it.jo.find('div.tabContainer');
                ...
                remove: function() {
                    if (this.jo) {
                        this.body.off();
                        this.tabs.off();
                        this.jo && this.jo.remove();
                        this.jo = null;
                        $('#dialogOk, #dialogCancel').unbind();
                        jive.ui.dialog.isVisible = false;
                    }
                }
            },
            forms: {
                ...
                remove: function() {
                    var forms = jive.ui.forms;
                    $.each(forms, function(prop) {
                        var value = forms[prop];
                        value.name && value.remove();
                    });
                }
            },
            colorpicker: {
            ...
            modal: {
                ...
                show: function(whoHasFocus) {
                    ...
                    if(!$('#jive_modal').length) {
                        this.jo = $('<div/>', {id: '#jive_modal'}).css({
                            display:"none",
                            position:"absolute",
                            zIndex: 9999,
                            top:0,
                        ...
                        jive.getReportContainer().append(this.jo);
                        this.jo.on(clickEventName,function(evt){
                            jive.ui.modal.hide();
                            evt.preventDefault();
                        })
                    }
        ...
        init: function(report) {
            ...
            jive.elements[report.id] = {};
            jive.selectors[report.id] = {};

            !$('#jive_components').length && this.createJiveElements();
        },
        createJiveElements: function() {
            jive.isDashboard = $('body').is('.dashboardViewFrame');
            /*
             Scrolable table headers
             */
            if (jive.isDashboard || $('div#reportViewFrame').length > 0) {
                jive.setScrollableHeader(jive.isDashboard);
            }

            /*
             Setup HTML
             */
            jive.$jiveComponents = $("<div/>", {id: "jive_components"}).attr("data-reportId", jive.reportInstance.id);
            $('body').append(jive.$jiveComponents);
            jive.$jiveComponents.empty();
            jive.$jiveComponents.append(templates);

            /*
             Event Handling
             */
            jive.reportInstance.config.container.on(clickEventName, function() {

                if (jive.ui.dialog.isVisible) {
                    return;
                }

                // hide everything (dialog, marker, overlay, foobar)
                jive.hide();
                $('body').trigger('jive.inactive');
            });

            var jmenuitem;
            this.$menus = $('div#jive_menus, div#jive_dropdown');
            ...
            $('body').trigger('jive.initialized', [jive]);
        },
        initInteractiveElement: function(o, reportId, jqReportContainer){
            if(jive.elements[reportId][o.config.id]) {
                /*
                 * Check if report has been reloaded, if so we need to setup event
                 * listeners.
                 */
                if(!$.hasData(jqReportContainer.find('table.jrPage')[0])) {
                    jive.selectors[reportId] = {};
                }
            }

            jive.elements[reportId][o.config.id] = o;
            /*
             * Check if event listener has for selector already set.
             */
            if(!jive.selectors[reportId][o.config.parentId + "-" + o.config.selector]){

                jive.selectors[reportId][o.config.parentId + "-" + o.config.selector] = o.config.type;

                jqReportContainer.find('table.jrPage').on(clickEventName, o.config.selector, function(evt){
                    // keep html links functional
                    var $parent = $(evt.target).parent();
                    if ($parent.is('a') || $parent.hasClass("_jrHyperLink")) {
                        // let other listeners to catch this event
                        return;
                    }
                    var jo = $(this);
                    !$('#jive_components').length && jive.createJiveElements();
                    jive.selectInteractiveElement(jo, reportId);
                    return false;
                })
            }

            if(o.config.proxySelector && !jive.selectors[reportId][o.config.parentId + "-" + o.config.proxySelector]) {
                jive.selectors[reportId][o.config.parentId + "-" + o.config.proxySelector] = o.config.type;
        ...
        selectInteractiveElement: function(jo, reportId){
            if (jo && jo.is('.interactiveElement')) {
                ...
                jive.$jiveComponents.attr("data-reportId", reportId);

                var colData = jive.interactive.column.columnsData[reportId][jo.data('tableuuid')][jo.data('coluuid')];

                jive.selected = {
                    ie: jive.elements[reportId][jo.data('coluuid')],
                 ...
                 */
                var headerCols = jive.activeContainer.find('table.jrPage .jrcolHeader[data-coluuid=' + jo.data('coluuid') + ']'),
                ...
                // attempt to find the zoom level on the report
                var parent = jive.selected.ie;
                while (parent.parent && (parent = parent.parent) && !parent.zoom) {}
                if (jive && jive.ui && jive.ui.scaleFactor) {
                    (jive.selected.dim.zoom = jive.ui.scaleFactor);
                } else {
                    // we are in the JRS mode, trying to get zoom level from the JRS report instance
                    parent.zoom && (jive.selected.dim.zoom = parent.zoom.level);
                }
        ...
        scale: function(scaleFactor){
            jive.ui.scaleFactor = scaleFactor;
            if(jive.selected && jive.selected.dim){
                jive.selected.dim.zoom = scaleFactor;
            }
        },
        ...
        remove: function(report) {
            if (jive.$jiveComponents.attr("data-reportId") === report.id) {
                jive.$menus.off();
                jive.ui.dialog.remove();
                jive.ui.marker.remove();
                jive.ui.overlay.remove();
                jive.ui.foobar.remove();
                jive.ui.forms.remove();
                jive.ui.colorpicker.remove();
                jive.ui.modal.remove();
                jive.$jiveComponents.remove();
            }
        }
    };
