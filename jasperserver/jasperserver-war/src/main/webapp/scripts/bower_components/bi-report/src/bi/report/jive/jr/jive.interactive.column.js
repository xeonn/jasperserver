...
/**
 * @version $Id: jive.interactive.column.js 2833 2014-11-09 15:19:31Z ktsaregradskyi $
 */


define(["jquery.ui", "./jive", "text!jr.jive.chartSelector.tmpl","bundle!CommonBundle", "csslink!jr.jive.highcharts.vm.css"], function($, jive, chartSelectorTemplates, i18n) {
    ...
    jive.interactive.column = {
        ...
        init: function(report) {
            ...
            $.each(report.components.table, function() {
                ...
                if(!it.genericPropertiesInitialized) {
                    ...
                    it.initColumns(this, report.id, report.config.container);
                }
            });
        },

        scale: function(scaleFactor){
            jive.scale(scaleFactor);
        },

        initColumns: function(Table, reportId, jqReportContainer){
            ...
            var colsData = it.columnsData[reportId][tableUuid] = {};
            ...
            $.each(Table.columns, function(k, thisColumn){
                thisColumn && jive.initInteractiveElement(thisColumn, reportId, jqReportContainer);
            });
        },
        ...
        handleServerError: function(result) {
            ...
            if (!this.errorDialog) {
                this.errorDialog = $(chartSelectorTemplates).find('.jive_chartTypeSelector').css({
                    "z-index": 500,
                    "width": 400,
                    "min-height": 150
                });
            ...
            this.errorDialog.find(".body").html("<span>" + result.devmsg + "</span>");
            this.errorDialog.find(".title").html(i18n["dialog.exception.title"]);

            this.errorDialog.show().position({
                of: 'body',
                at: 'center center',
                my: 'center center'
            });
        ...
        showCurrencyDropDown: function(){
            jive.selected.form.inputs['currencyBtn1-jr-uuid-for-ui-element-0'].showOptions();
        },
        applyCurrencyFormat: function(args) {
            var it = this,
                input = jive.selected.form.inputs['currencyBtn1-jr-uuid-for-ui-element-0'],
                cSymbol = $('#formatPattern-jr-uuid-for-ui-element-0').data('cSymbol');

            $('#formatPattern-jr-uuid-for-ui-element-0').children().each(function (i, optElem) {
                var opt = $(optElem);
                ...
                if (args.val && args.val.length > 0) {
                    ...
                    $('#formatPattern-jr-uuid-for-ui-element-0').data('cSymbol', args.val);
                }
            });
        ...
        togglePercentageFormat: function(){
            ...
            $('#formatPattern-jr-uuid-for-ui-element-0').children().each(function (i, optElem) {
                var opt = $(optElem);
                opt.text(it.numberFormat.addRemovePercentageForNumber(opt.text(), jive.selected.form.inputs['percentageBtn-jr-uuid-for-ui-element-0'].value));
                opt.val(it.numberFormat.addRemovePercentage(opt.val(), jive.selected.form.inputs['percentageBtn-jr-uuid-for-ui-element-0'].value));
            });
        },
        toggleCommaFormat: function(){
            ...
            $('#formatPattern-jr-uuid-for-ui-element-0').children().each(function (i, optElem) {
                var opt = $(optElem);
                opt.text(it.numberFormat.addRemoveThousandsSeparator(opt.text(), jive.selected.form.inputs['commaBtn-jr-uuid-for-ui-element-0'].value));
                opt.val(it.numberFormat.addRemoveThousandsSeparator(opt.val(), jive.selected.form.inputs['commaBtn-jr-uuid-for-ui-element-0'].value));
            });
        },
        addDecimal: function(){
            var it = this;
    ...
    jive.interactive.column.columnFilterForm = {
        ...
        jc: {},
        elements: [
            [[{type:'radio',id:'clearFilter-jr-uuid-for-ui-element-0',label: jive.i18n.get('column.filterform.clearfilter.true.label'), value:'true'}]],
            [
                [{type:'radio',id:'clearFilter-jr-uuid-for-ui-element-0',label: jive.i18n.get('column.filterform.clearfilter.false.label'), value:'false',colspan:4}],
                [
                    {type:'list', id:'filterTypeOperator-jr-uuid-for-ui-element-0', values:[]},
                    {type:'text', id:"fieldValueStart-jr-uuid-for-ui-element-0", value:''},
                    {type:'label', value:'and'},
                    {type:'text', id:"fieldValueEnd-jr-uuid-for-ui-element-0", value:''}
                ]
            ]
        ],
        onCreate:function(jo){
            ...
            it.jc.filterStart = $('#fieldValueStart-jr-uuid-for-ui-element-0');
            it.jc.filterEnd = $('#fieldValueEnd-jr-uuid-for-ui-element-0').prop('disabled',true);
            it.jc.filterType = $('#filterTypeOperator-jr-uuid-for-ui-element-0').change(function(){
            ...
            $('input[name="clearFilter-jr-uuid-for-ui-element-0"]').change(function(){
        ...
        onShow:function(){
            ...
            $('input[name="clearFilter-jr-uuid-for-ui-element-0"][value="'+filterOff+'"]').prop("checked",true);
        ...
        submit: function() {
            var metadata = jive.selected.ie.config.filtering.filterData,
                ...
                filterData = {
                    ...
                    fieldValueStart: jive.selected.form.jo.find('input[name="fieldValueStart-jr-uuid-for-ui-element-0"]').val(),
                    filterTypeOperator: jive.selected.form.jo.find('select[name="filterTypeOperator-jr-uuid-for-ui-element-0"]').val(),
                    clearFilter: jive.selected.form.jo.find('input[name="clearFilter-jr-uuid-for-ui-element-0"]:checked').val()
                };

            if(!jive.selected.form.jo.find('input[name="filterValueEnd"]').is(':hidden')){
                filterData.fieldValueEnd = jive.selected.form.jo.find('input[name="fieldValueEnd-jr-uuid-for-ui-element-0"]').val();
            }
        ...
        remove: function() {
            if (this.isInitialized) {
                $('#filterTypeOperator-jr-uuid-for-ui-element-0').off("change");
                $('input[name="clearFilter-jr-uuid-for-ui-element-0"]').off("change");
                this.jo = null;
            }
        }
    };
    ...
    jive.interactive.column.basicFormatForm = {
        ...
        elements: [
            [
                [
                    {type: 'text', id: 'headingName-jr-uuid-for-ui-element-0', label: jive.i18n.get('column.formatHeaderForm.headingName.label'), value: '', colspan: 4}
                ]
            ],
            // common format
            [
                [
                    {type: 'list', id: 'fontName-jr-uuid-for-ui-element-0', label: jive.i18n.get('column.formatforms.fontName.label'), values: [], freeText: true, size: 6, rowspan: 2},
                    {type: 'list', id: 'fontSize-jr-uuid-for-ui-element-0', label: jive.i18n.get('column.formatforms.fontSize.label'), values: [], freeText: true, size: 6, rowspan: 2},
                    {
                        type: 'buttons',
                        label: jive.i18n.get('column.formatforms.styleButtons.label'),
                        items: [
                            {type: 'checkbox', id: 'fontBold-jr-uuid-for-ui-element-0', value: 'bold', bIcon: 'boldIcon'},
                            {type: 'checkbox', id: 'fontItalic-jr-uuid-for-ui-element-0', value: 'italic', bIcon: 'italicIcon'},
                            {type: 'checkbox', id: 'fontUnderline-jr-uuid-for-ui-element-0', value: 'underline', bIcon: 'underlineIcon'}
                        ]
                    },
                    {
                        type: 'buttons',
                        ...
                        items: [
                            {type: 'backcolor', id: 'fontBackColor-jr-uuid-for-ui-element-0', bIcon: 'backgroundColorIcon', title: jive.i18n.get('column.formatforms.fontBackColor.title'), drop: true, showTransparent: true, styleClass: 'wide'},
                            {type: 'color', id: 'fontColor-jr-uuid-for-ui-element-0', bIcon: 'fontColorIcon', title: jive.i18n.get('column.formatforms.fontColor.title'), drop: true}
                        ]
                    }
                ],
                [
                    {
                        ...
                        items: [
                            {type: 'radio', id: 'fontAlign-jr-uuid-for-ui-element-0', value: 'Left', bIcon: 'leftIcon'},
                            {type: 'radio', id: 'fontAlign-jr-uuid-for-ui-element-0', value: 'Center', bIcon: 'centerIcon'},
                            {type: 'radio', id: 'fontAlign-jr-uuid-for-ui-element-0', value: 'Right', bIcon: 'rightIcon'}
                        ]
                    }
                ]
            ],
            ...
            [
                [
                    {type: 'list', id: 'formatPattern-jr-uuid-for-ui-element-0', label: jive.i18n.get('column.formatCellsForm.formatPattern.label'), freeText: true, values: [], colspan: 2, size: 4, rowspan: 2},
                    {
                        type: 'buttons',
                        id: 'numberFormatButtons-jr-uuid-for-ui-element-0',
                        ...
                        items: [
                            ...
                            {type: 'checkbox', id: 'percentageBtn-jr-uuid-for-ui-element-0', fn: 'togglePercentageFormat', value: '', bIcon: 'percentageIcon'},
                            {type: 'checkbox', id: 'commaBtn-jr-uuid-for-ui-element-0', fn: 'toggleCommaFormat', value: '', bIcon: 'commaIcon'},
                            {type: 'action', id: 'increaseDecimalsBtn-jr-uuid-for-ui-element-0', fn: 'addDecimal', value: '', bIcon: 'increaseDecimalsIcon'},
                            {type: 'action', id: 'decreaseDecimalsBtn-jr-uuid-for-ui-element-0', fn: 'remDecimal', value: '', bIcon: 'decreaseDecimalsIcon'}
                        ]
                    }
                ],
                [
                        ...
                        items: [
                            {
                                type: 'dropdown',
                                drop: true,
                                id: 'currencyBtn1-jr-uuid-for-ui-element-0',
        ...
        onCreate: function(jo) {
            ...
            this.$columnSwitchers = $("#colnext, #colprev");

            // events for applyTo selector
            this.$applyTo.on('change', function(evt) {
            ...
            this.$columnSwitchers.on(jive.clickEventName, function(evt) {
                if(this.className.indexOf('disabled') < 0){
                    var colIdx = jive.selected.ie.config.columnIndex;
                    jive.selected.form.onBlur();

                    if(this.id == 'colnext') {
                        jive.selected.ie = jive.elements[jive.reportInstance.id][it.getNextVisibleColUuid(colIdx)];
                    } else {
                        jive.selected.ie = jive.elements[jive.reportInstance.id][it.getPrevVisibleColUuid(colIdx)];
                    }
        ...
        onGenericShow: function(applyToVal) {
            ...
            inputs['fontBold-jr-uuid-for-ui-element-0'].set(metadata.fontBold);
            inputs['fontItalic-jr-uuid-for-ui-element-0'].set(metadata.fontItalic);
            inputs['fontUnderline-jr-uuid-for-ui-element-0'].set(metadata.fontUnderline);

            inputs['fontAlign-jr-uuid-for-ui-element-0'].set(metadata.fontHAlign);
            if (!isFromCache) {
                if (applyToVal === 'heading') {
                    inputs['headingName-jr-uuid-for-ui-element-0'].set(jive.decodeHTML(metadata.headingName));
                }
                inputs['fontName-jr-uuid-for-ui-element-0'].set(jive.decodeHTML(metadata.fontName));
            } else {
                if (applyToVal === 'heading') {
                    inputs['headingName-jr-uuid-for-ui-element-0'].set(metadata.headingName);
                }
                inputs['fontName-jr-uuid-for-ui-element-0'].set(metadata.fontName);
            }
            inputs['fontSize-jr-uuid-for-ui-element-0'].set(metadata.fontSize);
            inputs['fontColor-jr-uuid-for-ui-element-0'].set(metadata.fontColor);
            inputs['fontBackColor-jr-uuid-for-ui-element-0'].set(metadata.fontBackColor, metadata.mode);
            ...
            if (applyToVal !== 'heading') {
                ...
                if(dataType && (dataType == 'numeric' || dataType == 'date' || dataType == 'time')) {
                    ...
                    $('#formatPattern-jr-uuid-for-ui-element-0').html(htm.join(''));
                    if (!isFromCache) {
                        inputs['formatPattern-jr-uuid-for-ui-element-0'].set(jive.decodeHTML(metadata.formatPattern));
                    } else {
                        inputs['formatPattern-jr-uuid-for-ui-element-0'].set(metadata.formatPattern);
                    }
                    jo.find('tr').show();
                    if (dataType == 'numeric') {
                        jo.find('tr:eq(0)').children('td:last').css('visibility','visible');
                        jo.find('tr:eq(1)').children('td:last').css('visibility','visible');
                        inputs['percentageBtn-jr-uuid-for-ui-element-0'].set(false);
                        inputs['commaBtn-jr-uuid-for-ui-element-0'].set(false);
                    } else {
        ...
        getActionData: function(prevApplyTo) {
            ...
            result = {
                ...
                editTextElementData:{
                    ...
                    fontName: jive.escapeFontName(inputs['fontName-jr-uuid-for-ui-element-0'].get()),
                    fontSize: inputs['fontSize-jr-uuid-for-ui-element-0'].get(),
                    fontBold: inputs['fontBold-jr-uuid-for-ui-element-0'].get(),
                    fontItalic: inputs['fontItalic-jr-uuid-for-ui-element-0'].get(),
                    fontUnderline: inputs['fontUnderline-jr-uuid-for-ui-element-0'].get(),
                    fontHAlign: inputs['fontAlign-jr-uuid-for-ui-element-0'].get(),
                    fontColor: inputs['fontColor-jr-uuid-for-ui-element-0'].get(),
                    fontBackColor: inputs['fontBackColor-jr-uuid-for-ui-element-0'].getBackColor(),
                    mode: inputs['fontBackColor-jr-uuid-for-ui-element-0'].getModeValue(),
            ...
            switch(val) {
                case 'heading':
                    result.editTextElementData.headingName = inputs['headingName-jr-uuid-for-ui-element-0'].get();
                    break;
                case 'tabletotal':
                case 'detailrows':
                    result.editTextElementData.formatPattern = inputs['formatPattern-jr-uuid-for-ui-element-0'].get();
                    break;
                case 'groupheading':
    ...
    jive.interactive.column.columnConditionalFormattingForm = {
        ...
        templateElements: [
            {type:'label', value:''},
            {type:'list', id:'conditionTypeOperator-jr-uuid-for-ui-element-0', values:[]},
            {type:'text', id:'conditionStart-jr-uuid-for-ui-element-0', value:'', wrapClass: 't_wrap'},
            {type:'label', value:'and', wrapClass: 't_wrap', tdClass: 'condition_between_start'},
            {type:'text', id:'conditionEnd-jr-uuid-for-ui-element-0', value:'', wrapClass: 't_wrap'},
            {
                type: 'buttons',
                items: [
                    {type:'checkbox',id:'conditionFontBold-jr-uuid-for-ui-element-0',value:'bold',bIcon:'boldIcon', isTripleState: true},
                    {type:'checkbox',id:'conditionFontItalic-jr-uuid-for-ui-element-0',value:'italic',bIcon:'italicIcon', isTripleState: true},
                    {type:'checkbox',id:'conditionFontUnderline-jr-uuid-for-ui-element-0',value:'underline',bIcon:'underlineIcon', isTripleState: true},
                    {type:'backcolor',id:'conditionFontBackColor-jr-uuid-for-ui-element-0',bIcon:'backgroundColorIcon',title:jive.i18n.get('column.formatforms.fontBackColor.title'), drop: true, showTransparent: true, showReset: true, styleClass: 'wide'},
                    {type:'color',id:'conditionFontColor-jr-uuid-for-ui-element-0',bIcon:'fontColorIcon',title:jive.i18n.get('column.formatforms.fontColor.title'), drop: true, showReset: true}
                ]
            },
            {
                type: 'buttons',
                items: [
                    {type:'action', id:'conditionMoveUp-jr-uuid-for-ui-element-0', fn: 'conditionMoveUp', bIcon: 'moveUpIcon', btnClass: 'plain'},
                    {type:'action', id:'conditionMoveDown-jr-uuid-for-ui-element-0', fn: 'conditionMoveDown', bIcon: 'moveDownIcon', btnClass: 'plain'}
                ]
            },
            {type:'button', id:'conditionRemove-jr-uuid-for-ui-element-0', bIcon:'deleteIcon', fn: 'removeFormatCondition', btnClass: 'plain', tdClass: 'last'}
        ],
        elements: [
            [
                [
            ...
            [
                ...
                [
                    {type:'label'},
                    {type:'button', id:'conditionAdd-jr-uuid-for-ui-element-0', bLabel: jive.i18n.get('column.conditionalFormatting.condition.add'), fn: 'addFormatCondition', btnClass: 'plain', nowrap: true},
        ...
        onCreate: function(jo){
            ...
            this.form.on('change', 'select[name=conditionTypeOperator-jr-uuid-for-ui-element-0]', function(evt) {
                var self = $(this),
                    row = self.closest('tr'),
                    conditionEnd = row.find('input[name^=conditionEnd-jr-uuid-for-ui-element-0]'),
                    conditionStart = row.find('input[name^=conditionStart-jr-uuid-for-ui-element-0]');
            ...
            this.form.on('rowchange', 'table', function(evt) {
                $(this).find('tr.jive_condition').each(function(i, v) {
                    $(this).find('.jive_textLabel:first div:first').text(i+1);
                });
            });

            this.form.find('table:eq(1)').addClass('conditionList').find('tr:last').addClass('add');
        },
        columnChanged: function() {
            var existingApplyToText = $('#applyTo :selected').text(),
                newOption;
        ...
        onGenericShow:function(applyToVal){
            var it = this,
                ...
                isFromCache = false,
                addButton = jive.selected.form.jo.find('div.jive_inputbutton[bname=conditionAdd-jr-uuid-for-ui-element-0]'),
                metadata;
        ...
        addFormatCondition: function(jo, conditionData, isFromCache) {
            if (!jo.is('.disabled')) {
                ...
                row.find('select[name=conditionTypeOperator-jr-uuid-for-ui-element-0]').append(htm.join('')).trigger('change');
                ...
                if(conditionType === 'date') {
                    ...
                    row.find('input[name=conditionStart-jr-uuid-for-ui-element-0]').datetimepicker(pickerOptions);
                    row.find('input[name=conditionEnd-jr-uuid-for-ui-element-0]').datetimepicker(pickerOptions);
                    ...
                    row.find('input[name=conditionStart-jr-uuid-for-ui-element-0]').timepicker(timePickerOptions);
                    row.find('input[name=conditionEnd-jr-uuid-for-ui-element-0]').timepicker(timePickerOptions);
                } else if (conditionType === 'boolean') {
                    row.find('input[name=conditionStart-jr-uuid-for-ui-element-0]').prop('disabled', true);
                    row.find('input[name=conditionStart-jr-uuid-for-ui-element-0]').closest('td').hide();
                }

                if (conditionData) {
                    row.find('select[name=conditionTypeOperator-jr-uuid-for-ui-element-0]').val(conditionData.conditionTypeOperator).trigger('change');
                    inputs[row.find('input[name=conditionEnd-jr-uuid-for-ui-element-0]').attr('id')].set(conditionData.conditionEnd);

                    if (conditionType === 'text' && isFromCache) {
                        inputs[row.find('input[name=conditionStart-jr-uuid-for-ui-element-0]').attr('id')].set(conditionData.conditionStart);
                    } else {
                        inputs[row.find('input[name=conditionStart-jr-uuid-for-ui-element-0]').attr('id')].set(jive.decodeHTML(conditionData.conditionStart));
                    }

                    inputs[row.find('.jive_inputbutton[bname^=conditionFontBold-jr-uuid-for-ui-element-0]').attr('bname')].set(conditionData.conditionFontBold);
                    inputs[row.find('.jive_inputbutton[bname^=conditionFontItalic-jr-uuid-for-ui-element-0]').attr('bname')].set(conditionData.conditionFontItalic);
                    inputs[row.find('.jive_inputbutton[bname^=conditionFontUnderline-jr-uuid-for-ui-element-0]').attr('bname')].set(conditionData.conditionFontUnderline);
                    inputs[row.find('.jive_inputbutton[bname^=conditionFontColor-jr-uuid-for-ui-element-0]').attr('bname')].set(conditionData.conditionFontColor);
                    inputs[row.find('.jive_inputbutton[bname^=conditionFontBackColor-jr-uuid-for-ui-element-0]').attr('bname')].set(conditionData.conditionFontBackColor, conditionData.conditionMode);
                } else {
                    inputs[row.find('.jive_inputbutton[bname^=conditionFontBold-jr-uuid-for-ui-element-0]').attr('bname')].set(null);
                    inputs[row.find('.jive_inputbutton[bname^=conditionFontItalic-jr-uuid-for-ui-element-0]').attr('bname')].set(null);
                    inputs[row.find('.jive_inputbutton[bname^=conditionFontUnderline-jr-uuid-for-ui-element-0]').attr('bname')].set(null);
                    inputs[row.find('.jive_inputbutton[bname^=conditionFontColor-jr-uuid-for-ui-element-0]').attr('bname')].set(null);
                    inputs[row.find('.jive_inputbutton[bname^=conditionFontBackColor-jr-uuid-for-ui-element-0]').attr('bname')].set(null, null);
                }
        ...
        getActionData: function(prevApplyTo) {
            ...
            jive.selected.form.jo.find('table:eq(1) tr.jive_condition').each(function(i, v) {
                ...
                actionData.conditionalFormattingData.conditions.push({
                    conditionStart: inputs[row.find('input[name=conditionStart-jr-uuid-for-ui-element-0]').attr('id')].get(),
                    conditionEnd: inputs[row.find('input[name=conditionEnd-jr-uuid-for-ui-element-0]').attr('id')].get(),
                    conditionTypeOperator: inputs[row.find('select[name=conditionTypeOperator-jr-uuid-for-ui-element-0]').attr('id')].get(),
                    conditionFontBold: inputs[row.find('.jive_inputbutton[bname^=conditionFontBold-jr-uuid-for-ui-element-0]').attr('bname')].get(),
                    conditionFontItalic: inputs[row.find('.jive_inputbutton[bname^=conditionFontItalic-jr-uuid-for-ui-element-0]').attr('bname')].get(),
                    conditionFontUnderline: inputs[row.find('.jive_inputbutton[bname^=conditionFontUnderline-jr-uuid-for-ui-element-0]').attr('bname')].get(),
                    conditionFontColor: inputs[row.find('.jive_inputbutton[bname^=conditionFontColor-jr-uuid-for-ui-element-0]').attr('bname')].get(),
                    conditionFontBackColor: inputs[row.find('.jive_inputbutton[bname^=conditionFontBackColor-jr-uuid-for-ui-element-0]').attr('bname')].getBackColor(),
                    conditionMode: inputs[row.find('.jive_inputbutton[bname^=conditionFontBackColor-jr-uuid-for-ui-element-0]').attr('bname')].getModeValue()
                });
            });
