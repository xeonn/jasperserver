...
*/
package com.jaspersoft.buildomatic.loader;
...
public class MondrianFoodMartLoader {
        ...
        for (String arg : args) {
            if (arg.equals("-verbose")) {
				...
				if (EncryptionEngine.isEncrypted(password)) {
					...
					Integer encBlockSize = ksManager.getDefaultEncBlockSize();
					String cryptoTransf = ksManager.getDefaultEncTranformation();
					password = EncryptionEngine.decrypt(ksManager.getBuildKey(), password,
							new EncryptionProperties(encBlockSize, cryptoTransf));
				}
            ...
            } else if (arg.startsWith("-outputInitFile=")) {
           	 	outputInitFile = arg.substring("-outputInitFile=".length());
        ...
        throws Exception
    {
        ...
        try {
            ...
            while ((line = bufferedReader.readLine()) != null) {
                ...
                // make oracle columns uppercase
                if (dialect.getDatabaseProduct() == Dialect.DatabaseProduct.ORACLE) {
                	columnNames = columnNames.toUpperCase();
                }
    ...
    private void loadFromSqlInserts() throws Exception {
        ...
        try {
            ...
            // don't quote oracle or db2, so that new tables will match default case (uppercase)
            if (dialect.getDatabaseProduct() == Dialect.DatabaseProduct.ORACLE ||
            		dialect.getDatabaseProduct() == Dialect.DatabaseProduct.DB2) {
            	toQuoteChar = "";
            }
            while ((line = bufferedReader.readLine()) != null) {
                ++lineNumber;
    ...
    private void executeInitSQL() throws Exception {
   	 if (outputInitFile == null) {
   		 return;
   	 }
       final File file = new File(outputInitFile);
       LOGGER.info("running outputInitFile: " + outputInitFile);
       ...
       if (file.canRead()) {
           ...
           while ((line = bufferedReader.readLine()) != null) {
               ...
               LOGGER.info("running outputInitFile statement: " + line);
